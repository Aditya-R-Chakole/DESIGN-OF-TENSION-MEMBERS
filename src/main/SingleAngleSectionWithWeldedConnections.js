...
function make_design(input, output){
  ...
  console.log("Minimum Area Required : ", minAreaRequired, " sq.mm");                          //printing the information in console
  output.min_area_required = minAreaRequired;

  /*---------------------Strength of Weld Per Unit Length------------------------*/
  let StrengthOfWeldperunitLength = (ThroatThickness*UltimateStressOfSteel)/(1.73*PSF_Weld);   //Strength of weld per unit length
  let LengthOfWeld = (FactoredLoad)/(StrengthOfWeldperunitLength);                             //Length of weld requuired
  ...
  for( let i=0;i<size;i++ ){
    ...
    if( AreaOfAngle >= minAreaRequired ){
      ...
      let Tdg = (AreaOfAngle*YeildStressOfSteel)/(PSF_Yeilding);    //Design Strength due to Yielding
      
      console.log("Strength due to Yeilding : ", Tdg/1000, " kN");  //printing the information in console

      /*---------------------Design Strength due to Rupture------------------------*/
      let alpha = 0.8;                                                              //Considering constent alpha = 0.8
      let Anc = (AngleLength - (AngleThickness/2))*(AngleThickness);                //net area of the connected leg
      let Ago = (AngleWidth - (AngleThickness/2))*(AngleThickness);                 //gross area of the outstanding leg
      let An = Anc+Ago;                                                             //Net area of the total cross-section
      let Tdn = (alpha*An*UltimateStressOfSteel)/(PSF_UltimateStress);              //Design Strength due to Rupture
      ...
      let Avg = (Math.max(LengthOfWeldInUpperSide, LengthOfWeldInLowerSide))*(AngleThickness)*2;                             //minimum gross area in shear along bolt line
      let Avn = (Math.max(LengthOfWeldInUpperSide, LengthOfWeldInLowerSide))*(AngleThickness)*2;                             //net area in shear along bolt line
      let Atn = (AngleLength)*(AngleThickness);                                                                              //net area in tension from the bolt hole to the toe of the angle
      let Atg = (AngleLength)*(AngleThickness);                                                                              //minimum gross area in tension from the bolt hole to the toe of the angle

      let Tdb1 = ( (0.9*Avn*UltimateStressOfSteel)/(1.73*PSF_UltimateStress) )+( (YeildStressOfSteel*Atg)/(PSF_Yeilding) );  //Strength due to Block Shear, For tension yield and shear fracture
      let Tdb2 = ( (Avg*YeildStressOfSteel)/(1.73*PSF_Yeilding) )+( (0.9*UltimateStressOfSteel*Atn)/(PSF_UltimateStress) );  //Strength due to Block Shear, For tension fracture and shear yield

      let Tdb = Math.min(Tdb1, Tdb2);  //Design Strength due to Block Shear
      ...
      console.log("Design Tensile Strength Of Angle : ", DesignTensileStrengthOfAngle/1000, " kN");             //printing the information in console
...
function SingleAngleSectionWithWeldedConnections() {
    ...
    <div >
      ...
      <article>
        <form>
          ...
          <p className='heading_light'
            style = {{position:'absolute', top:'747px', left:'655px', fontSize:'37px'}}>Take according to <b>IS800 Table 5</b> (clause 5.4.1)</p>
...
export default SingleAngleSectionWithWeldedConnections;